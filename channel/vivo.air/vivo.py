...
class vivo(Channel):
    ...
    def getPoco(self, PocoType = Channel.eCheckPoint["None"]):
        ...
        if (PocoType == self.eCheckPoint["OrdinarySplash_Exist"]):        # 普通开屏_存在
            # return None
            return [poco(nameMatches = '(.*)id/tt_splash_ad_gif'), poco(text='我的汤姆猫')]
        ...
        if (PocoType == self.eCheckPoint["OrdinaryBanner_Exist"]):        # 普通Banner_存在
            return [poco(text="广告"), poco("com.outfit7.mytalkingtom.vivo:id/banner_layout")]
    ...
    def getImage(self, ImageType = Channel.eCheckPoint["None"]):
        ...
        if (ImageType == self.eCheckPoint["OrdinaryInterstitial_Exist"]):  # 普通插屏_存在
            return [Template(r"tpl1567505512972.png", record_pos=(0.005, 0.424), resolution=(1080, 2248)), Template(r"tpl1569480761122.png", record_pos=(0.001, 0.36), resolution=(1080, 2244))]
